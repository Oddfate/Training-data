...
def handle_json(path):
    ...
    try:
        ...
        if not text.strip():
            print(f"⚠️ JSON appears empty: {path}")
            return

        embed_and_store(text, path, "json")
        print(f"🧾 JSON extracted and embedded: {path}")
...
def process_folder(folder_path):
    ...
    for root, _, files in os.walk(folder_path):
        for name in tqdm(files, desc=f"🔄 Processing: {os.path.basename(root)}"):
